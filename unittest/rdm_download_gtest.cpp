...
public:
    ...
    MOCK_METHOD2(rdmDwnlUnInstallApp, void(CHAR* pDwnlPath, CHAR* pAppPath));
    //MOCK_METHOD(void, rdmDwnlUnInstallApp, void(CHAR*, CHAR*), ());
...
extern "C" {
    ...
    void rdmDwnlUnInstallApp(CHAR *pDwnlPath, CHAR *pAppPath) { gMock->rdmDwnlUnInstallApp(pDwnlPath, pAppPath); }
    }
...
TEST_F(RdmDownloadTest, Test_LegacyDownloadSuccessAfterRetry) {
    RDMAPPDetails appDetails;
    SetupRdmAppDetails(appDetails);
    int dlStatus = RDM_SUCCESS;
    ...
    EXPECT_CALL(*gMock, checkFileSystem(_)).WillOnce(Return(true));
    EXPECT_CALL(*gMock, rdmDownloadMgr(_))
        .Times(2) // Modify as needed for how many retries before success
    ...
    EXPECT_CALL(*gMock, createDir(_)).WillOnce(Return(0));
