...
PEER_COMM_DAT="/etc/dropbear/interchip"
...
generateDownloadUrl()
{
    ...
    if [ "$flag" = 1 ]; then
        ...
        FQDN=`echo "$serverUrl" | awk -F/ '{print $3}'`
    else
        if [ -f $EnableOCSPStapling ] || [ -f $EnableOCSP ]; then
            CURL_CMD="$TLS $IF_OPTION -fgL $CURL_OPTION '%{http_code}\n' -o \"$DOWNLOAD_LOCATION/$file\" \"$url\" --cert-status --connect-timeout $CURL_TLS_TIMEOUT -m 600"
        else
...
sendDownloadRequest()
{
    ...
    curl_cmd=`echo $curl_request | sed -e "s|-H.*-w|-H 'AuthorizationHeader' -w|g; s#devicecert_1.*-fgL#devicecert_1.pk12<masked> -fgL#g; s#staticXpkiCr.*-fgL#staticXpkiCrt.pk12<masked> -fgL#g"`
    FQDN=`echo "$downloadUrl" | awk -F/ '{print $3}'`
    while [ $status -ne 0 ]
    do
        log_msg "sendDownloadRequest: CURL_CMD: curl ${curl_cmd}"
        TLSRet=` exec_curl_mtls "$curl_request" "RDM" "$FQDN"`
...
sendAppDownloadRequest()
{
    ...
    if [ "$flag" = 1 ]; then
          ...
          TLSRet=` exec_curl_mtls "$curl_request" "log_msg"`
    fi
    http_code=$(awk -F\" '{print $1}' $HTTP_CODE)
    case $TLSRet in
     35|51|53|54|58|59|60|64|66|77|80|82|83|90|91)
       tlsLog "CERTERR, RDM, $TLSRet, $FQDN"
       t2ValNotify "certerr_split" "RDM, $TLSRet, $FQDN"
    ;;
    esac
    if [ $TLSRet -ne 0 ];then
    ...
    if [ "$DEVICE_TYPE" = "broadband" ]; then
        ...
        if [ "$UseCodebig" = 1 ]; then
            ...
            if [ "$DEVICE_TYPE" = "mediaclient" ]; then
                ...
                if [ $skipcodebig -eq 0 ]; then
                    ...
                    do
                        ...
                        generateDownloadUrl $downloadFile $downloadUrl $UseCodebig
                        sendAppDownloadRequest "$CURL_CMD" $UseCodebig
                        ret=$TLSRet
                        http_code=$(awk -F\" '{print $1}' $HTTP_CODE)
                        if [ "$http_code" = "200" ]; then
            ...
            if [ $skipdirect -eq 0 ]; then
                ...
                log_msg "applicationDownload: Direct App download blocked"
                return 1
            fi
            ...
            if [ $ret -ne 0 ] && [ "$http_code" == "000" ]; then
                if [ "$DEVICE_TYPE" = "mediaclient" ];then
                    ...
                    log_msg "applicationDownload: Not a mediaclient device. Hence not eligible for Codebig connection"
                    exit 4
                fi
            else
                if [ $ret -ne 0 ]; then
                    log_msg "applicationDownload: httpcode is $http_code. Hence not eligible for Codebig connection"
                fi
            fi
        fi
    fi
